...
| #   | 议题           | 要讨论的内容                                       | 状态     |
| --- | -------------- | -------------------------------------------------- | -------- |
| 33  | 配置分层策略   | base.env + 环境.env + .env.local 的分层覆盖        | ✅ 已确定 |
| 34  | 目录结构       | 后端配置放 backend/config/，部署配置放 deploy/     | ✅ 已确定 |
| 35  | 敏感信息管理   | .env.local 不提交，生产敏感信息通过 CI/服务器管理  | ✅ 已确定 |
| 36  | 前端配置       | Vite 约定：.env / .env.development / .env.production | ✅ 已确定 |
...
| #   | 议题           | 要讨论的内容                          | 状态     |
| --- | -------------- | ------------------------------------- | -------- |
| 37  | CI/CD          | GitHub Actions？手动部署？            | 待讨论   |
| 38  | 域名与 SSL     | 域名规划、证书申请（Let's Encrypt？） | 待讨论   |
| 39  | 部署架构       | 单机 Docker？云服务器配置？           | 待讨论   |
| 40  | Docker Compose | compose.yml + compose.dev/prod.yml 分层 | ✅ 已确定 |
| 41  | Nginx 配置     | 反向代理 + 前端静态资源               | ✅ 已确定 |
| 42  | 数据备份       | PostgreSQL 备份策略、频率             | 待讨论   |
| 43  | 监控告警       | 需要吗？MVP 阶段怎么做？              | 待讨论   |
| 44  | 日志收集       | 直接看容器日志？还是 ELK？            | 待讨论   |
...
| #   | 议题        | 要讨论的内容                   | 状态   |
| --- | ----------- | ------------------------------ | ------ |
| 45  | 软删除策略  | 哪些数据软删除？保留多久？     | 待讨论 |
| 46  | 数据归属    | 任务的 owner 是家长还是孩子？  | 待讨论 |
| 47  | AI 调用策略 | 失败重试？降级方案？成本控制？ | 待讨论 |
| 48  | 离线支持    | 需要考虑吗？PWA？              | 待讨论 |
...
- **tech-impl-guide.md**：技术实施指南（已确定议题的详细方案）
- **mvp_0_1_db_schema_and_migration_v3.md**：数据库设计
- **solo_developer_methodology.md**：开发方法论
